...
namespace DTAClient.Domain.Singleplayer
{
    ...
    public class CampaignHandler
    {
        ...
        private void WriteBonuses(string path)
        {
            var iniFile = new IniFile(path);

            for (int i = 0; i < Bonuses.Count; i++)
            {
                iniFile.SetStringValue("Bonuses", i.ToString(), Bonuses[i].ININame);
            }

            Bonuses.ForEach(b => b.WriteToIni(iniFile));
        ...
        {
            var iniFile = new IniFile(Path.Combine(ProgramConstants.GamePath, "INI", "CampaignBonuses.ini"));

            var bonusesSection = iniFile.GetSection("Bonuses");
            if (bonusesSection == null)
                return;

            foreach (var kvp in bonusesSection.Keys)
            {
                var bonusSection = iniFile.GetSection(kvp.Value);
                if (bonusSection == null)
                    continue;

                var bonus = Bonus.FromIniSection(bonusSection, kvp.Value);
                Bonuses.Add(bonus);
            }
        }
        ...
        private void SanityCheckCampaigns()
        {
            ...
            foreach (var bonus in Bonuses)
            {
                ...
                {
                    MessageBox.Show("Failed to find unlock mission for Bonus: " + bonus.UnlockFromMission, "Bonus Configuration Error");
                }
            }
        }
        ...
        public void WriteFilesForMission(Mission mission, DifficultyRank selectedDifficultyLevel, Dictionary<int, bool> globalFlagInfo, Bonus bonus, bool isCheatSession)
        {
            ...
            using (StreamWriter swriter = new StreamWriter(File.OpenWrite(ProgramConstants.GamePath + ProgramConstants.SPAWNER_SETTINGS), Encoding.GetEncoding(1252)))
            {
                ...
                if (bonus != null)
                    swriter.WriteLine("BonusName=" + bonus.ININame);
            ...
            IniFile difficultyIni = new IniFile(ProgramConstants.GamePath + DifficultyIniPaths[ingameDifficultyLevel], Encoding.GetEncoding(1252));

            if (copyMapsToSpawnmapINI)
            {
                IniFile mapIni = new IniFile(ProgramConstants.GamePath + mission.Scenario, Encoding.GetEncoding(1252));
                ...
                bonus?.Difficulty.WriteToFile(mapIni, "PlayerNormal", true);
                ...
                // Set global-specific INI values
                foreach (var globalIniValue in mission.GlobalSpecificINIValues)
                {
                    int globalFlagIndex = GlobalVariables.FindIndex(gv => gv.InternalName.Equals(globalIniValue.GlobalVariableName, StringComparison.OrdinalIgnoreCase));
                    if (globalFlagIndex >= 0 && globalFlagInfo != null && globalFlagInfo.ContainsKey(globalFlagIndex) && globalFlagInfo[globalFlagIndex])
                    {
                        mapIni.SetStringValue(globalIniValue.INISection, globalIniValue.INIKey, globalIniValue.INIValue);
                    }
                }
